...
- **Canlı bildirimler** (`LiveNotificationModule.kt`, Android 16 *Live Updates* API): `Notification.ProgressStyle` + `FLAG_PROMOTED_ONGOING` + `setShortCriticalText` ile **Samsung One UI 8 Now Bar**, kilit ekranı ve durum çubuğu çipinde görünür; Android 15 ve altında klasik ongoing + progress bildirimine düşer.
  ...
  - **Ders zili**: OBS programı `lessons_json` olarak native tarafa yazılır; ders başlamadan 45 dk önce "Sıradaki ders 10:30 · X · B302" geri sayımı, ders boyunca "Şu an: X · 11:20'e kadar · sıradaki Y" bitiş kronometresi. `LessonLiveReceiver` alarmı ders başlangıç/bitişlerine kurulur; günün son dersinden sonra kapanır. Ayarlar ve `/brief` ekranından açılır (varsayılan kapalı).
  - **Now Bar kartı**: Başlıklar emojisiz; kapsülün ikinci satırı (`shortCriticalText`) kalan süre ("7 sa 53 dk", "12 dk") ve dakikada bir tazelenir (Samsung sistem kronometresini kapalı kapsülde çizmez). Bildirime `Notification.Action` eklenince Samsung genişleyen kartta tam içeriği (alt metin, saniyeli kronometre, ilerleme çubuğu) ve düğmeleri çizer: namaz → *Vakitler / Kapat*, ders → *Program / Kapat*, otobüs → *Haritada aç*. "Kapat" kilit açmadan `LiveActionReceiver` üzerinden native tercihi kapatır; ayar ekranları öne gelince JS anahtarını bununla eşitler.
  - **Samsung kilit ekranı notu**: One UI'da global "Kilitliyken içeriği gizle" açıkken Now Bar, üçüncü taraf kartlarda `VISIBILITY_PUBLIC`/`publicVersion`'a bakmadan yalnızca uygulama adını gösterir; kanal `lockscreenVisibility` isteğini de çerçeve sıfırlar. Tek çözüm kullanıcının uygulama bazlı "Kilitliyken içeriği göster veya gizle → Her zaman göster" seçimi; uygulama bunu tespit edip (`lockscreenContentHidden`) tek dokunuşla ilgili ayar sayfasına yönlendirir. Ders/vakit geçişleri için `SCHEDULE_EXACT_ALARM` izni istenir; verilmezse 10 dk pencereli alarm.
  - Neden `MediaStyle` değil: medya bildirimi gerçek bir `MediaSession` ister; Spotify/YouTube ile "aktif medya" yarışına girer, Bluetooth/kulaklık tuşlarını çalar ve Android 14+ oturumu çalmıyorsa medya alanında zaten göstermez. Google Haritalar'ın Now Bar'da yaptığı şey de bu değil, tam olarak yukarıdaki Live Updates (`ProgressStyle` + promoted ongoing) API'sidir.
